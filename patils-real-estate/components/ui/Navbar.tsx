    ...
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-8">
            {links.map((l) => (
              <Link
                ...
                style={{ color: pathname === l.href ? 'var(--teal)' : (isDarkText ? 'var(--navy)' : 'var(--white)') }}
              >
                {l.label}
                <span
                  className="absolute -bottom-1 left-0 h-px transition-all duration-300"
                  style={{ width: pathname === l.href ? '100%' : '0%', backgroundColor: isDarkText ? 'var(--teal)' : 'var(--white)' }}
                />
                <span 
                  className="absolute -bottom-1 left-0 h-px w-0 group-hover:w-full transition-all duration-300" 
                  style={{ backgroundColor: isDarkText ? 'var(--teal)' : 'var(--white)' }}
                />
              </Link>
            ))}
          </nav>
          ...
          <Link
            href="/contact"
            className={`hidden md:inline-flex items-center px-5 py-2 font-dm text-xs tracking-widest uppercase transition-all duration-500 ${isDarkText ? 'bg-[var(--teal)] text-[var(--white)] hover:bg-[var(--navy)]' : 'bg-[var(--white)] text-[var(--navy)] hover:bg-[var(--teal)] hover:text-[var(--white)]'}`}
          >
            Schedule Visit
          </Link>
          ...
          <button
            ...
            className={`md:hidden p-1 transition-colors duration-500 ${isDarkText ? 'text-[var(--navy)]' : 'text-[var(--white)]'}`}
          >
